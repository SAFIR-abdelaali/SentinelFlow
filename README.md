...
- The frontend (Next.js 15 + Tailwind CSS) will be available at [http://localhost:3000](http://localhost:3000)
- The backend (FastAPI + Uvicorn) will be available at [http://localhost:8000](http://localhost:8000)
...
2. **Set API URL (optional):**
    - By default, the frontend expects the backend at `http://localhost:8000` via the `NEXT_PUBLIC_API_URL` environment variable. You can set this in a `.env.local` file if needed.
